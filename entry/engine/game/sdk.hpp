...
__forceinline s_matrix get_view_matrix() {
	auto view_render = mem::Read1<uintptr_t>(baseAddress + off::viewRender);
	if (!view_render)
		return s_matrix{};

	auto pview_matrix = mem::Read1<uintptr_t>(view_render + off::viewMatrix);
	if (pview_matrix != 0) {
		return mem::Read1<s_matrix>(pview_matrix);
	}
	return s_matrix{};
}
...
public:
	...
	inline int weapon_id() {
		...
		return mem::Read1<int>(this->address + off::weaponHandle);
	}

	inline int ammo_in_clip() {
		if (!this->address) return -1;
		return mem::Read1<int>(this->address + off::weaponAmmo);
	}

	inline float bullet_speed() {
		if (!this->address) return 0.0f;
		return mem::Read1<float>(this->address + off::projectileSpeed);
	}

	inline float bullet_gravity() {
		if (!this->address) return 0.0f;
		return mem::Read1<float>(this->address + off::projectileScale);
	}
...
public:
	...
	c_player(DWORD_PTR pointer) {
		...
		this->bleed_out_state = mem::Read1<int>(pointer + off::bleedoutState);
	}
	...
	inline bool b_downed() {
		if (this->bleed_out_state > 0) {
			return true;
		}
		return false;
	}
	inline bool alive() {
		return mem::Read1<int>(this->address + off::bleedoutState) == 0 && mem::Read1<int>(this->address + off::lifeState) == 0;
	}
	int GetWeaponIndex()
	{
		uint64_t primary_weapons_latest = mem::Read1<uint64_t>(this->address + off::weaponHandle) & 0xFFFF;
		uint64_t primary_weapons_list = mem::Read1<uint64_t>(baseAddress + off::entityList + (primary_weapons_latest * 0x20));
		int weapon_index = mem::Read1<int>(primary_weapons_list + off::weaponIndex); // weapon name0x1574 0x0769b4b8
		return weapon_index;
	}

	inline c_weapon weapon() {
		auto weapon_pointer = mem::Read1<uintptr_t>(this->address + off::weaponHandle);
		weapon_pointer &= 0xffff;
		auto weapon_handle = mem::Read1<uintptr_t>(baseAddress + off::entityList + (weapon_pointer << 5));
		return c_weapon(weapon_handle);
	}

	inline bool IsSpectating() {
		if (!this->alive())
			return false;
		uint64_t SpectatorList = mem::Read1<uint64_t>(baseAddress + 0x6192c88);
		int PlayerData = mem::Read1<int>(this->address + 0x38);
		int SpecIndex = mem::Read1<int>(SpectatorList + PlayerData * 8 + 0x0954);
		uint64_t SpectatorAddr = mem::Read1<uint64_t>(baseAddress + off::entityList + ((SpecIndex & 0xFFFF) << 5));
		if (SpectatorAddr == this->address)
			return true;
		return false;
	}
	...
	inline std::string s_name() {
		...
		auto index = mem::Read1<uintptr_t>(this->address + off::nameIndex);

		uintptr_t NameOffset = mem::Read1<uintptr_t>(baseAddress + off::nameList + ((index - 1) * 24)); //NameList=0xd350450
		if (!NameOffset)
			return "AI";
		std::string PlayerName = mem::readString(NameOffset);
		return PlayerName;
	}
	...
	std::string GetOperator()
	{
		...
		uintptr_t pMode_Name = mem::Read1< uintptr_t >(this->get_address() + 0x0030);
		if (!pMode_Name) {
			return "";
		}
		auto ModelName = mem::readString(pMode_Name);
	...
	inline int i_health() {
		this->health = mem::Read1<int>(this->address + off::health);
		return this->health;
	}

	inline bool on_ground() {
		this->flags = mem::Read1<uint32_t>(this->address + off::flags);
	...
	inline int i_team() {
		this->team = mem::Read1<int>(this->address + off::teamNumber);
		return this->team;
	}

	inline int i_shield() {
		this->shield = mem::Read1<int>(this->address + off::shield);
		return this->shield;
	}

	inline int i_max_shield() {
		this->max_shield = mem::Read1<int>(this->address + off::maxHealth);
		return this->max_shield;
	}

	inline int i_shield_type() {
		this->shield_type = mem::Read1<int>(this->address + off::armorType);
	...
	inline vector3 mins() {// OFF_Collision_m_vecMins
		return mem::Read1<vector3>(this->address + off::collisionVecMins);
	}

	inline vector3 maxs() {
		return mem::Read1<vector3>(this->address + off::collisionVecMaxs);
	}

	inline int distance() {
		auto local_player = mem::Read1<uintptr_t>(baseAddress + off::localPlayer);
		auto local_dist = mem::Read1<vector3>(local_player + off::localOrigin);

		return (int)local_dist.distance(this->vec_origin());
	}

	inline vector3 vec_origin() const {
		return mem::Read1<vector3>(this->address + off::localOrigin);
	}
	inline vector3 r_view_angles() {
		return mem::Read1<vector3>(this->address + off::viewAngles);
	}

	inline vector3 camera_position() {
		return mem::Read1<vector3>(this->address + off::cameraOrigin);
	}
	...
	inline vector3 bone_position(uint32_t id) {
		uintptr_t bone_array = mem::Read1<uintptr_t>(this->address + off::bones);
		this->origin = this->vec_origin();

		vector3 bone = vector3();
		bone.x = mem::Read1<float>(bone_array + 0xCC + (id * 0x30)) + this->origin.x;
		bone.y = mem::Read1<float>(bone_array + 0xDC + (id * 0x30)) + this->origin.y;
		bone.z = mem::Read1<float>(bone_array + 0xEC + (id * 0x30)) + this->origin.z;
		return bone;
	}
	...
	inline bool visible(int index) {
		if (get_ms() >= (last_visible.last_check_time[index] + 10)) {
			float s_visible_time = mem::Read1<float>(this->address + off::lastVisibleTime);
	...
	inline vector3 punch_angles() {
		return mem::Read1<vector3>(this->address + off::punchAngles);
	}
	...
	inline vector3 velocity() const {
		return mem::Read1<vector3>(this->address + off::absVelocity);
	}
...
__forceinline std::string class_signifier(DWORD_PTR entity) {
	auto class_pointer = mem::Read1<uintptr_t>(entity + off::signifierName);
	
	return mem::readString(class_pointer);
}

inline std::string get_weapon_name(uint64_t playerEntity)
{
	uint64_t primary_weapons_latest = mem::Read1<uint64_t>(playerEntity + off::weaponHandle) & 0xFFFF;
	uint64_t primary_weapons_list = mem::Read1<uint64_t>(baseAddress + off::entityList + (primary_weapons_latest * 0x20));

	int weapon_index = mem::Read1<int>(primary_weapons_list + off::weaponIndex);
